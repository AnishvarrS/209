...
def openChatWindow():
    ...
    namelabel = Label(window, text= "Enter Your Name", font = ("Modern",10))
    namelabel.place(x=10, y=8)

    name = Entry(window,width =30,font = ("Modern",10))
    name.place(x=120,y=8)
    name.focus()

    connectserver = Button(window,text="Connect to SHARE Server",bd=1, font = ("Modern",10))
    connectserver.place(x=350,y=6)

    separator = ttk.Separator(window, orient='horizontal')
    separator.place(x=0, y=35, relwidth=1, height=0.1)

    labelusers = Label(window, text= "Active Users", font = ("Modern",10))
    labelusers.place(x=10, y=50)

    listbox = Listbox(window,height = 5,width = 67,activestyle = 'dotbox', font = ("Modern",10))
    ...
    connectButton=Button(window,text="Connect",bd=1, font = ("Modern",10))
    connectButton.place(x=282,y=160)

    disconnectButton=Button(window,text="Disconnect",bd=1, font = ("Modern",10))
    disconnectButton.place(x=350,y=160)

    refresh=Button(window,text="Refresh",bd=1, font = ("Modern",10))
    refresh.place(x=435,y=160)

    labelchat= Label(window, text="SHARE-WINDOW", font= ("Modern",10)) 
    labelchat.place(x=10, y=180)

    textarea= Text(window, width=67, height=6, font=("Modern",10))
    ...
    attach=Button(window,text="Attach",bd=1, font = ("Modern",10))
